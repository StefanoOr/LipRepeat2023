...
let rec string_of_decl_v = function
  | EmptyDecl -> ""
  | IntVar(x) -> "int " ^ x
  | DSeq(d1,d2) -> string_of_decl_v d1 ^ ";" ^ string_of_decl_v d2
  | IntArr(x,e) -> "int " ^ x ^ "[" ^ string_of_expr e ^ "]"  


let rec string_of_decl_p = function
  | EmptyDecl -> ""
  | Proc(f,x,c,e) -> "proc " ^ f ^ "(" ^ x ^ ") {" ^ string_of_cmd c ^ "}"
  | DSeq(d1,d2) -> string_of_decl_p d1 ^ ";" ^ string_of_decl_p d2
